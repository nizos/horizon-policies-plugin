(function() {
    ...
    angular
        ...
        .provider('massAutocompleteConfig', function () {
            ...
            config.position_autocomplete = function (container, target) {
                if (typeof target !== 'undefined') {
                    var textarea = document.getElementsByClassName("autocomplete_field")[0];

                    var cursor_position = textarea.value.length - textarea.value.lastIndexOf('\n');
                    var line_number = textarea.value.substr(0, textarea.selectionStart).split("\n").length;
                    // var autocomplete_offset_top = autocomplete_class.offsetTop;
                    // var autocomplete_offset_left = autocomplete_class.offsetLeft;


                    // var inputPosition = "autocomplete_field_1";
                    // var cursorPosition = inputPosition.slice(0, inputPosition.selectionStart).length;
                    var rect = textarea.getBoundingClientRect();

                    container[0].style.top = (line_number * 13) + 140 + 'px';
                    container[0].style.left = (cursor_position * 7) + 20 + 'px';
        ...
        .directive('massAutocomplete', [
            'massAutocompleteConfig', '$timeout', '$window', '$document', '$q',
        ...
        .directive('massAutocompleteItem',
            function () {
                return {
                    restrict: 'A',
                    require: [
        ...
        .controller('detailsController', [
            ...
            function($uibModalInstance, $log, $scope, $policy) {
                ...
                var suggestions = ["admin", "admin_required", "all", "and", "credential", "domain_id", "is_admin", "owner", "or", "project", "project_id", "role", "role:admin", "role:reader", "rule", "rule:admin_required", "rule:service_role", "rule:service_or_admin", "rule:token_subject", "rule:owner", "reader", "service_role", "service_or_admin", "system_scope", "system_scope:all", "system", "target", "target.credential.user_id", "target.domain.id", "target.domain_id", "target.group.domain_id", "target.limit.domain.id", "target.limit.project.domain_id", "target.limit.project_id", "target.trust.trustor_user_id", "target.trust.trustee_user_id", "target.project.domain_id", "target.project.id", "target.role.domain_id", "target.token.user_id", "target.user.id", "target.user.domain_id", "token", "token.domain.id", "token.project.domain.id", "token_subject", "trust.trustor_user_id", "user_id"];
                ...
                function provide_suggestion(term) {
                    var q = term.toLowerCase().trim();
                    var results = [];

                    for (var i = 0; i < suggestions.length && results.length < 5; i++) {
                        var suggestion = suggestions[i];
                        if (suggestion.toLowerCase().indexOf(q) === 0)
                ...
                $scope.autocomplete_options = {
                    suggest: provide_suggestion
                };


                function provide_suggestion_delimited(term) {
                    var delimiters = [
                        term.lastIndexOf('\n'),
                        term.lastIndexOf('\t'),
                        term.lastIndexOf('"'),
                    ...
                    var ix = Math.max(...delimiters),
                        ...
                        suggestions = provide_suggestion(rhs);
                ...
                $scope.ac_option_delimited = {
                    suggest: provide_suggestion_delimited
                };
